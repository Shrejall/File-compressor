...
// intialize a vector of size 128
// each index has 1 char and it's freq is 0 initially
...
// Traversing the Huffman tree, 0 if moved left and 1 if moved right
...
// dec is converted to bin and then appendede with leading zeroes to make it 8 bit
string huffman::decToBin(int inNum) {
    ...
    // eg-101 is appended with 5 leading zeroes to make it 8 bit bin number
...
void huffman::createTree() {
    ...
    while (tempPQ.size() != 1)
    {   
        //smaller or equal node is kept at left side
        left = tempPQ.top();
        tempPQ.pop();
        // greater node is kept at right side
        right = tempPQ.top();
        tempPQ.pop();
...
void huffman::saveEncodedFile() {
    ...
    in += (char)minHeap.size(); //store no. of char
    priority_queue <Node*, vector<Node*>, Compare> tempPQ(minHeap);
    while (!tempPQ.empty()) {
